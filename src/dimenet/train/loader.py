...
def get_loader(dataset: Dataset, cutoff=5., post_fn: Callable = to_tensor, **kwargs) -> DataLoader:
    collate_fn = partial(collate, cutoff=cutoff, post_fn=post_fn)
...
def collate(examples, cutoff: float, post_fn: Callable):
    ...
    for i, e in enumerate(examples):
        ...
        adj_matrices.append(sp.csr_matrix(D <= cutoff))
