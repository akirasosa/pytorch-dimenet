...
from dimenet.train.loader import AtomsBatch
...
class LRFinder(object):
    ...
    def _train_batch(self, iter_wrapper, accumulation_steps):
        ...
        for i in range(accumulation_steps):
            ...
            # Forward pass
            y_hat = self.model(inputs).squeeze(1)
            loss = self.criterion(y_hat, inputs.U0)
    ...
    def _validate(self, dataloader):
        ...
        with torch.no_grad():
            for inputs in dataloader:
                ...
                y_hat = self.model(inputs).squeeze(1)
                loss = self.criterion(y_hat, inputs.U0)
                running_loss += loss.item() * len(y_hat)
                total_pairs += len(y_hat)
